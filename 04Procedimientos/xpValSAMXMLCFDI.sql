...
FROM OPENXML (@iDatos, 'cfdi:Comprobante', 1) WITH ( [Version]   varchar(10),    
[Serie]    varchar(25),    
[Folio]    varchar(40),    
[Fecha]    varchar(19),    
[Sello]    varchar(max),    
...
FROM MensajeLista    
WHERE Mensaje = 80341   
IF @TrasladoTipoFactor IN (SELECT DISTINCT Factor FROM SATCatTasaOCuota)  
BEGIN  
IF @TrasladoTasaOCuota NOT IN (SELECT round(ValMax,2,1) FROM SATCatTasaOCuota WHERE round(ValMax,2,1) = CAST(@TrasladoTasaOCuota as varchar(10)) AND Factor = @TrasladoTipoFactor AND Traslado = 1)  
...
FROM MensajeLista    
WHERE Mensaje = 80363    
IF @TrasladoTipoFactor IN (SELECT DISTINCT Factor FROM SATCatTasaOCuota) and @TotTrasTipoFactor IN (SELECT DISTINCT Factor FROM SATCatTasaOCuota)  
BEGIN  
