...
GO
CREATE PROCEDURE spValidarXMLCFDI
(@Empresa		varchar(5),
@Sucursal		int,
@Modulo		varchar(5),
@ID			int,
...
EXEC sp_xml_preparedocument @iDatos OUTPUT, @XML, @NameSpace
/**********************************     E N C A B E Z A D O     ***************************************/
...
IF @Ok IS NULL AND @Version <> '3.3'
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ CAST(@Version as varchar(5))
FROM MensajeLista
WHERE Mensaje = 80300
...
BEGIN
IF @Fecha IS NULL
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')
FROM MensajeLista
WHERE Mensaje = 80301
ELSE
IF LTRIM(RTRIM(@Fecha)) NOT LIKE '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]'
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ @Fecha
FROM MensajeLista
WHERE Mensaje = 80302
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ ISNULL(@FormaPago,'')
FROM MensajeLista
WHERE Mensaje = 80304
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+CAST(@Subtotal as varchar(18))
FROM MensajeLista
WHERE Mensaje = 80309
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+CAST(@Descuento as varchar(18))
FROM MensajeLista
WHERE Mensaje = 80310
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ ISNULL(@Moneda,'')
FROM MensajeLista
WHERE Mensaje = 80312
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Moneda:'+ ISNULL(@Moneda,'')
FROM MensajeLista
WHERE Mensaje = 80313
ELSE
IF @Moneda NOT IN ('MXN','XXX') AND @TipoCambio < 0.000001
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Moneda:'+@Moneda+' Valor: '+ CAST(@TipoCambio as varchar(18))
FROM MensajeLista
WHERE Mensaje = 80314
ELSE
IF @Moneda = 'MXN' AND @TipoCambio <> 1.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Moneda:'+@Moneda+' Valor: '+ CAST(@TipoCambio as varchar(18))
FROM MensajeLista
WHERE Mensaje = 80315
END
...
ELSE
IF @Total < 0.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + CAST(@Total as varchar(18))
FROM MensajeLista
WHERE Mensaje = 80317
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TipoComprobante,'')
FROM MensajeLista
WHERE Mensaje = 80319
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@MetodoPago,'')
FROM MensajeLista
WHERE Mensaje = 80320
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ ISNULL(@LugarExpedicion,'')
FROM MensajeLista
WHERE Mensaje = 80322
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ ISNULL(@TipoRelacion,'')
FROM MensajeLista
WHERE Mensaje = 80323
ELSE
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ ISNULL(@UUIDRelacionado,'')
FROM MensajeLista
WHERE Mensaje = 80324
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ ISNULL(@RFC,'')
FROM MensajeLista
WHERE Mensaje = 80325
ELSE
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: '+ISNULL(@RegimenFiscal,'')
FROM MensajeLista
WHERE Mensaje = 80326
END
IF @Ok IS NULL
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@RfcReceptor,'')
FROM MensajeLista
WHERE Mensaje = 80327
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@ResidenciaFiscal,'')
FROM MensajeLista
WHERE Mensaje = 80328
END
IF @Ok IS NULL AND NOT EXISTS(SELECT ClaveUsoCFDI FROM SATCatUsoCFDI WHERE ClaveUsoCFDI = @UsoCFDI)
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@UsoCFDI,'')
FROM MensajeLista
WHERE Mensaje = 80329
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@CptoClaveProdServ,'')
FROM MensajeLista
WHERE Mensaje = 80330
IF @Ok IS NULL
IF @CptoCantidad < 0.000001
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@CptoCantidad,'')
FROM MensajeLista
WHERE Mensaje = 80332
IF @Ok IS NULL
IF NOT EXISTS(SELECT ClaveUnidad FROM SATCatClaveUnidad WHERE ClaveUnidad = @CptoClaveUnidad)
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@CptoClaveUnidad,'')
FROM MensajeLista
WHERE Mensaje = 80333
IF @Ok IS NULL
IF @CptoValorUnitario < 0.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(CAST(@CptoValorUnitario AS VARCHAR(50)),'')
FROM MensajeLista
WHERE Mensaje = 80336
IF @Ok IS NULL
IF @CptoImporte < 0.000001
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(CAST(@CptoImporte AS VARCHAR(50)),'')
FROM MensajeLista
WHERE Mensaje = 80337
IF @Ok IS NULL AND @CptoDescuento IS NOT NULL
IF @CptoDescuento < 0.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(CAST(@CptoDescuento AS VARCHAR(50)),'')
FROM MensajeLista
WHERE Mensaje = 80338
FETCH NEXT FROM cConcepto INTO @CptoClaveProdServ, @CptoNoIden, @CptoCantidad, @CptoClaveUnidad, @CptoUnidad, @CptoDescripcion, @CptoValorUnitario, @CptoImporte, @CptoDescuento
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TrasladoBase,'')
FROM MensajeLista
WHERE Mensaje = 80339
IF @TrasladoImpuesto NOT IN ('001','002','003')
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TrasladoImpuesto,'')
FROM MensajeLista
WHERE Mensaje = 80340
IF @TrasladoTipoFactor NOT IN (SELECT Descripcion FROM SATCatTipoFactor WHERE Descripcion = @TrasladoTipoFactor)
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TrasladoTipoFactor,'')
FROM MensajeLista
WHERE Mensaje = 80341
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TrasladoTasaOCuota,'')
FROM MensajeLista
WHERE Mensaje = 80342
IF @TrasladoImporte < 0.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TrasladoImporte,'')
FROM MensajeLista
WHERE Mensaje = 80343
FETCH NEXT FROM cTraslado INTO @TrasladoBase, @TrasladoImpuesto, @TrasladoTipoFactor, @TrasladoTasaOCuota, @TrasladoImporte
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + CAST(@RetencionBase as varchar(25))
FROM MensajeLista
WHERE Mensaje = 80344
IF @RetencionImpuesto NOT IN ('001','002','003')
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@RetencionImpuesto,'')
FROM MensajeLista
WHERE Mensaje = 80345
IF @RetencionTipoFactor NOT IN (SELECT Descripcion FROM SATCatTipoFactor WHERE Descripcion = @RetencionTipoFactor)
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@RetencionTipoFactor,'')
FROM MensajeLista
WHERE Mensaje = 80346
IF @RetencionTasaOCuota NOT IN (SELECT ValMax FROM SATCatTasaOCuota WHERE ValMax = CAST(@RetencionTasaOCuota as varchar(10)) AND Factor = @RetencionTipoFactor AND Retencion = 1)
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@RetencionTasaOCuota,'')
FROM MensajeLista
WHERE Mensaje = 80347
IF @RetencionImporte < 0.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + CAST(@RetencionImporte as varchar(25))
FROM MensajeLista
WHERE Mensaje = 80348
FETCH NEXT FROM cRetencion INTO @RetencionBase, @RetencionImpuesto, @RetencionTipoFactor, @RetencionTasaOCuota, @RetencionImporte
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@NumeroPedimento,'')
FROM MensajeLista
WHERE Mensaje = 80349
FETCH NEXT FROM cInfoAduanera INTO @NumeroPedimento
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@CtaPredialNumero,'')
FROM MensajeLista
WHERE Mensaje = 80350
FETCH NEXT FROM cCtaPredial INTO @CtaPredialNumero
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@ParteClaveProdServ,'')
FROM MensajeLista
WHERE Mensaje = 80351
IF @Ok IS NULL
IF @ParteCantidad < 0.000001
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@ParteCantidad,'')
FROM MensajeLista
WHERE Mensaje = 80353
IF @Ok IS NULL
IF @ParteValorUnitario < 0.000000
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@ParteValorUnitario,'')
FROM MensajeLista
WHERE Mensaje = 80356
IF @Ok IS NULL
IF @ParteImporte < 0.000001
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@ParteImporte,'')
FROM MensajeLista
WHERE Mensaje = 80357
FETCH NEXT FROM cParte INTO @ParteClaveProdServ, @ParteNoIden, @ParteCantidad, @ParteUnidad, @ParteDescripcion, @ParteValorUnitario, @ParteImporte
END
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TotTrasImpuesto,'')
FROM MensajeLista
WHERE Mensaje = 80362
IF NOT EXISTS(SELECT Descripcion FROM SATCatTipoFactor WHERE Descripcion = @TotTrasTipoFactor)
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TotTrasTipoFactor,'')
FROM MensajeLista
WHERE Mensaje = 80363
...
SELECT @Ok = Mensaje, @OkRef = Descripcion + ' Movimiento: '+ISNULL(@Mov,'')+' '+ISNULL(@MovID,'')+' Valor: ' + ISNULL(@TotTrasTasaOCuota,'')
FROM MensajeLista
WHERE Mensaje = 80364
...
IF NOT EXISTS (SELECT * FROM MovTipoCFDFlex WHERE Modulo = @Modulo AND Mov = @Mov AND Comprobante = 'CFDI_3.3_Impuestos_Locales') 
